    ...
    def call(self, system: str, user: str, max_tokens: int = 4096) -> LLMResponse:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                # 503/500-class errors are transient overload on Google's side — retry with backoff.
                if attempt == MAX_RETRIES:
                    raise
                delay = RETRY_BASE_DELAY * (2 ** (attempt - 1))
                print(f"  [Gemini overloaded, attempt {attempt}/{MAX_RETRIES} — retrying in {delay}s]")
                time.sleep(delay)
